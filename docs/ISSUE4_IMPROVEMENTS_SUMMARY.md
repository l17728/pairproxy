...
Issue #4 实现了**健康检查认证支持**，支持对不提供 `/health` 端点的大型 LLM 服务商（如 Anthropic Claude、OpenAI、Huawei Cloud 等）进行健康检查。

本文档总结了初始实现之后的改进工作，包括日志增强和测试扩展。
...
| Huawei Cloud MaaS | AKSK Signing | ⏳ 框架就位，待实现 |
...
#### 3. Huawei Cloud AKSK 支持 (Low Priority)
当前使用 Bearer 作为降级方案，完整支持需要：
- HMAC-SHA256 签名计算
- 自定义 Authorization 头格式
- 时间戳和 nonce 处理
...
| Commit | 消息 | 时间 |
|--------|------|------|
| 476a6ce | fix(health): add provider-aware authentication for health checks | 2026-04-03 |
| 0210ce9 | improve(health): add comprehensive logging and expanded test coverage | 2026-04-03 |
