...
下表列出现有 `protocol_converter.go` 的已实现功能与本设计要求的差距：

| 功能点 | 现有实现状态 | 本设计要求 | 优先级 |
|--------|------------|-----------|--------|
| 基础文本请求转换 | ✅ 已实现 | 完整 | — |
| system 字段转换 | ✅ 已实现 | 完整 | — |
| 流式 stream_options 注入 | ✅ 已实现 | 完整 | — |
| temperature/top_p/stop_sequences 转换 | ❌ 未实现 | §2.2 | P1 |
| top_k 丢弃 | ❌ 未实现 | §2.2 | P1 |
| tools 字段转换（input_schema→parameters） | ❌ 未实现 | §2.5 | P0（工具调用） |
| tool_choice 转换 | ❌ 未实现 | §2.6 | P0（工具调用） |
| assistant 消息中 tool_use 块 → tool_calls | ❌ 未实现 | §2.4.3 | P0（工具调用） |
| user 消息中 tool_result 块 → tool 消息 | ❌ 未实现 | §2.4.4 | P0（工具调用） |
| 图片内容块转换（base64/url） | ❌ 未实现 | §2.4.2 | P1（多模态） |
| prefill 末尾 assistant 消息处理 | ❌ 未实现 | §2.4.6 | P1 |
| 非流式响应 id 前缀替换 | ❌ 未实现 | §3.1 | P2 |
| 非流式响应 tool_calls 转换 | ❌ 未实现 | §3.4 | P0（工具调用） |
| 非流式 usage cached_tokens 拆分 | ❌ 未实现 | §3.5 | P1（计费准确性） |
| 非流式 model 字段映射回 Anthropic 名 | ❌ 未实现 | §3.6 | P2 |
| 非流式 stop_reason tool_calls/content_filter | ⚠️ 部分（仅 stop/length） | §3.3 | P1 |
| 流式：input_tokens 准确值（缓冲策略） | ❌ 未实现（始终为0） | §4.1 | P0（计费关键） |
| 流式：工具调用转换 | ❌ 未实现 | §4.4 | P0（工具调用） |
| 流式：finish_reason 正确传递到 stop_reason | ⚠️ 硬编码 end_turn | §4.3 | P1 |
| 流式：cached_tokens → cache_read_input_tokens | ❌ 未实现 | §4.7 | P1 |
| 错误响应格式转换 | ❌ 未实现 | §5.1 | P1 |
| 模型名映射配置 | ❌ 未实现 | §6.1 | P2 |
| thinking/redacted_thinking 丢弃 | ❌ 未实现 | §6.8 | P1 |
